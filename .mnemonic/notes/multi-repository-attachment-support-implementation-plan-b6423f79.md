...
updatedAt: '2026-05-22T19:04:08.973Z'
role: plan
alwaysLoad: false
...
Federated read-only project attachments: link external repositories as knowledge sources. Attached repo notes participate in recall, summaries, list, get, memory_graph, and relationship previews. No writes, no auto-discovery, explicit add/remove tools.
...
Consolidate, prune, remember, update, move_memory, forget, and relate/unrelate must never operate on attached vaults. These tools use `collectVisibleNotes`, `searchOrder`, and `findNote` for vault enumeration. We add an explicit filter or a separate `searchOrderForReads` (excluding attachments for mutation paths):

- `searchOrder(cwd)` — includes attachments (used by recall, summary, list, get, graph)
- `searchOrderMutable(cwd)` — excludes attachments (used by consolidate, prune, remember, update, move_memory, forget, relate/unrelate)
...
- [ ] 16. recall, project_memory_summary, list, get, memory_graph, recent_memories: verify attached notes appear

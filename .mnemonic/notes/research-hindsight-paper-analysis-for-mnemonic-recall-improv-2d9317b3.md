...
updatedAt: '2026-04-24T18:08:47.588Z'
role: research
alwaysLoad: false
...
**How**: Store a derived JSON file per vault (gitignored, like projections) with term→document frequency map. Refresh on sync or when note count changes. Could be a `vault.storage` abstraction.
...
**How**: Detect temporal cues in query (e.g., "recent", "last week", "2025"), compute date range, boost notes whose `updatedAt` falls within range. Could be combined with existing temporal interpretation for history enrichment.
...
**What**: After fusion, apply a local cross-encoder (e.g., `cross-encoder/ms-marco-MiniLM-L-6-v2` via Ollama if available) to top candidates.
...
**How**: Requires Ollama model support for cross-encoders (currently unclear if Ollama supports cross-encoder architectures). Would be optional and best-effort.
**Fits mnemonic?** Maybe — depends on Ollama cross-encoder availability. If not available locally, this may require external API fallback, which conflicts with local-first design.
**VERDICT: Defer** until Ollama cross-encoder support is verified.
...
**How**: Would require LLM extraction pipeline, structured fact schema, background processing — significant architectural addition.
...
**VERDICT: Defer** — high cost, unclear benefit for current use case.

## Recommended Priority Order

1. **Graph spreading activation** — highest value for lowest cost. Uses existing data.
2. **RRF replacing additive scoring** — cleaner semantics, less tuning.
3. **Pre-computed BM25/IDF cache** — performance improvement for rescue path.
4. **Temporal retrieval boost** — leverages existing temporal metadata.
5. **Cross-encoder reranking** — after verifying Ollama support.
6. **Observation synthesis** — only if corpus grows and raw ingestion becomes common.

...
describe("vault-routing", () => {
    ...
    try {
      const response = await callLocalMcpResponse(
        ...
        "remember",
        {
          ...
          content:
            "Project-vault writes should commit locally without pushing unpublished branches by default.",
          tags: ["integration"],
  ...
  it("rewrites project metadata when moving a global note into a project vault", async () => {
    ...
    try {
      ...
      const moveText = await callLocalMcp(
        vaultDir,
        "move_memory",
        {
          id: noteId,
      ...
      const movedNote = await readFile(
        path.join(repoDir, ".mnemonic", "notes", `${noteId}.md`),
        "utf-8",
      );
    ...
    try {
      const rememberText = await callLocalMcp(
        ...
        "remember",
        {
          ...
          content:
            "Created with project context so it should remain project-associated when moved to the main vault.",
          tags: ["integration"],
  ...
  it("keeps note visibility coherent when moving a note from main to project and back", async () => {
    ...
    try {
      ...
      const moveToProject = await callLocalMcpResponse(
        vaultDir,
        "move_memory",
        {
          id: noteId,
      ...
      const moveBackToMain = await callLocalMcpResponse(
        vaultDir,
        "move_memory",
        {
          id: noteId,

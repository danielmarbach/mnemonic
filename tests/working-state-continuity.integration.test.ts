...
describe("working-state continuity", () => {
  it("recall filters by lifecycle temporary", async () => {
    ...
    try {
      ...
      const tempResponse = await callLocalMcpResponse(
        vaultDir,
        "recall",
        {
          query: "plan",
      ...
      const permResponse = await callLocalMcpResponse(
        vaultDir,
        "recall",
        {
          query: "decision",
  ...
  it("recovery workflow: orientation then temporary recall", async () => {
    ...
    try {
      ...
      const orientationResponse = await callLocalMcpResponse(
        vaultDir,
      ...
      const orientationResults = ProjectSummaryResultSchema.parse(
        orientationResponse.structuredContent,
      );
      expect(orientationResults.orientation.primaryEntry.title).toBe("Project overview");

      // Step 2: Recovery of working state
      const recoveryResponse = await callLocalMcpResponse(
        vaultDir,
        "recall",
        {
          query: "plan",
